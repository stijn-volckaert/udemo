...
//active?
function byte AllInstalled(){  //returns 0 for true, 1 for bad gen, 2 for bad guid, 3 bad file, 4 for not installed!
  local byte bRetVal;
  if (binstalled==0)
    return 4;
  bRetVal = IsMisMatch();
...
function bool IsDefPkg(){ //checks own package name:
   local string temp;
   temp=PackageName;
   if (instr(temp,".")!=-1)
      temp=left(temp,instr(temp,"."));
...
function bool IsDefaultPackage (string PkgName){    //seperated so can have other uses!
   Switch (PkgName){ //huge case statement system :)
...
{
  return class'DemoSettings'.default.Displaymode==0||(class'DemoSettings'.default.DisplayMode==1&&!bPBIShows)||(class'DemoSettings'.default.DisplayMode==2&&bInstalled==0);
}
